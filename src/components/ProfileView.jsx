...
export default function ProfileView({
  ...
  // Local dropdown state to keep UI concerns here
  ...
  return (
    <ThemeProvider initialSeason={profile?.season} initialMode={profile?.mode}>
          ...
          >
            <GearIcon
              ...
              className={`transition-colors group-hover:text-[var(--color-primary)] ${
                showDropdown ? "text-[var(--color-primary)]" : "text-[var(--color-text)]"
              }`}
              animate={spinMotion}
            />
            {showDropdown && (
      ...
      <div className="max-w-xl mx-auto p-8 space-y-6 mt-20 bg-[var(--color-surface)] round-xl shadow-md" style={{ fontFamily: "var(--font-sans)" }}>
        {profile.banner && (
          <div className="relative mb-4 -mx-8 -mt-8">
            ...
            {currentUser?.id === profile.id && (
              ...
              >
                <PencilIcon width={20} height={20} className="text-[var(--color-text)] hover:text-[var(--color-primary)]" />
              </button>
            )}
          </div>
        )}
        ...
        <div className="flex flex-col mt-20">
          <span style={{ fontFamily: "var(--font-garamond)" }} className="italic font-bold text-lg ml-4">
            and I Quote
          </span>
          <span className="text-md mt-1 text-center" style={{ fontFamily: "var(--font-garamond)" }}>
            “{profile.bio}”
          </span>
        </div>

        <div className="flex gap-8 mt-8 mb-8 ml-4">
          <span className="text-md font-semibold">{10} Following</span>
          <span className="text-md font-semibold">{10} Followers</span>
        </div>

        <div className="sticky top-0 z-50 bg-[var(--color-surface)] pt-4 flex justify-between items-end w-full gap-0 mt-8 relative">
          {["stories", "favorites", "fan-favorite"].map((tab) => (
            <button
              key={tab}
          ...
          <span
            ...
            style={{
              width: "33.3333%",
              left: activeTab === "stories" ? "0%" : activeTab === "favorites" ? "33.3333%" : "66.6666%",
          ...
          <div className="mt-8 space-y-6">
            {!currentUser ? (
              ...
              favoritePosts.map((post) => (
                <Post key={post.id} post={post} isFavorited={true} onToggleFavorite={() => onToggleFavorite(post.id)} />
              ))
            )}
          </div>
        )}
